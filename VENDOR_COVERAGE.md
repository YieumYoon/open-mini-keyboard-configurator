...
| Vendor LED swatches | `LED_color_N` object/style strings | `led-colors`, `led --color swatch-N` | LED color entry | Static palette extracted for swatches with confirmed RGB styles. `LED_color_34`, `LED_color_43`, and `LED_color_55` exist as object names but have no adjacent RGB style in extracted strings, so they are intentionally not exposed as color aliases. |
...
| Model picker / related boards | model strings `2KEY` through `15+3KEY`, `Widget::Set_Keyboard_*` handlers, `Identify_KeyBoard_style()` byte dispatcher | `vendor-models`, `vendor-models --handlers`, `vendor-models --routes` | not shown | Static catalog plus read-only route decoding. Current layout helpers target the tested `12+2KEY` board; handler list also shows internal layouts without public model strings. |
| Brand/resource variant clues | bundle id `COM.LQKJ.KEYBOARD.MINI-KEYBOARD`, strings `MINI_KEYBOARD`, `ANTICATER`, `ANTICATER_MINI`, `KB_*`, `BT_*`, `LAN_KD` QR image names | documented only | not shown | Static provenance/variant clues. Useful for matching rebranded apps or hardware families, but not a positive hardware identity by itself. |
| Physical UI slot surface | `pushButton_K1` through `pushButton_K27`, BK variants, `k1_left` through `k4_right`, `Layer1` through `Layer3` | `slots`, `vendor-models --handlers` | Layout pages | Static. Shows this binary carries UI support for layouts beyond the connected `12+2KEY` board; untested variant slot meanings should be fingerprinted before writing. |
| Bundle/app provenance | Qt/QMake Info.plist, Qt 5.12.9, bundled hidapi 0.12.0, Photoshop XMP dates in embedded PNG resources | documented only | none | Static. Asset metadata ranges from 2022-02-25 through 2024-09-06 and suggests an accumulating shared vendor app rather than one clean single-product build. |
